...
We now need to find out the precise subset of users that are actually vertexes.
...
```{r}
nyms <- V(g)$name
nyms <- setDT(data.frame(nyms))

sum(is.na(merge(nyms, users, by.x = 'nyms', by.y = 'User', all.x = T, all.y = T)$Role))
sum(is.na(merge(nyms, users, by.x = 'nyms', by.y = 'User', all.x = T)$Role))

merge(nyms, users, by.x = 'nyms', by.y = 'User', all.x = T, all.y = T)

merge(nyms, users, by.x = 'nyms', by.y = 'User', all.x = T)                           # I don't have an entry in users table for some vertex

is.na(merge(nyms, users, by.x = 'nyms', by.y = 'User', all.x = T)$TotalStacked)
```

From the previous reasoning I'm left with the following options:

1. Using for the graph only the users for which I have an entry in Users table;
2. Using for the graph also the users that have a vertex but do not have a corresponding entry in the users table.

The consequence of 1 is that I'm left with 5208 users, I have to rebuild a graph with those; the consequence of 2 is that I do not have anagraphic information for some users (~300), nor I have the Role or the StackedAmount. The second option doesn't seem to be a problem, we just need to understand why I'm left with less users in the users table after the scraping (because the list through which we loop is the list of all the unique users that posted or commented at least once).
...
```{r}
#simple_g <- set_vertex_attr(simple_g, "type", index = contrib, value = rep("contrib", length(contrib)))

#simple_g <- set_vertex_attr(simple_g, "type", index = valid_nyms, value = filtered_roles)
```

```{r}
#plot(simple_g, layout = layout_as_bipartite)
```
...
plot(simplify(g_first_posts, remove.multiple = T, remove.loops = T),
  vertex.label = NA,
