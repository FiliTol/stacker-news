...
def get_item_page(
    item_code,
):  # TODO this function could be bundled-in the function scraping_user.get_profile()
...
def detect_item_job(page):
    ...
    try:
        collect_button = page.find(
            "a", class_="btn btn-primary", target="_blank", tabindex="0"
        ).get_text()
...
def detect_item_link(page):
    ...
    try:
        collect_link = page.find(
            "a", class_="item_link__4cWVs", target="_blank"
        ).get_text()
...
def detect_item_bounty(page):
    ...
    try:
        collect_bounty = page.find(
            "div", class_="px-3 py-1 d-inline-block bg-grey-medium rounded text-success"
        ).get_text()
        return True
    except:
        return False
...
def detect_title(item, page):
    ...
    try:
        title = page.find(
            "a", class_="item_title__FH7AS text-reset me-2", href=f"/items/{item}"
        )
